...
def reconcile_countries_by_name(plot_countries, gdp_countries): #返回在世行有GDP数据的绘图库国家代码字典，以及没有世行GDP数据的国家代码集合
    ...
    dict={}
    set0=set()
    for plot_countries_code in plot_countries:  #从plot_countries中取出国家代码
        if plot_countries[plot_countries_code] in gdp_countries:  #判断某国家是否在世行有数据
            dict[plot_countries_code] = plot_countries[plot_countries_code]
        else:
            set0.add(plot_countries_code)
    tuple=(dict, set0)
    return tuple
...
def build_map_dict_by_name(gdpinfo, plot_countries, year):
    ...
    for plot_countries_code in plot_countries:
        for isp_country_code in gdpinfo:
            if gdpinfo[isp_country_code]["Country Name"]==plot_countries[plot_countries_code]:  #判断是否存在数据
                gdp_notes=''
                for y in range(1960, 2016):
                    gdp_notes+=gdpinfo[isp_country_code]['%d'%(y)]  #将所有的年份的GDP数据相加返还
                if gdp_notes=='':  #判断当年各国家是否有GDP数据
                    set1.add(plot_countries_code)  #得到在世行GDP数据中完全没有记录的绘图库国家代码的集合
                else:
                    if gdpinfo[isp_country_code][year]!='':  # 当年有GDP数据
                        gdp_number = math.log10(float((gdpinfo[isp_country_code][year])))  # GDP结果转换为以10为基数的对数格式
                        countries_year_gdp[plot_countries_code] = gdp_number
                    else:
                        set2.add(plot_countries_code)
                continue
    ...
    for countries in not_in_countries:
        set1.add(countries)  #绘图库里不在世界银行数据里的国家
    tuple=(countries_year_gdp,set1,set2)
    return tuple
...
def render_world_map(gdpinfo, plot_countries, year, map_file): #将具体某年世界各国的GDP数据(包括缺少GDP数据以及只是在该年缺少GDP数据的国家)以地图形式可视化
    ...
    worldmap_chart.title = '世界银行%s年全球国家GDP数据'%(year)  #添加标题
    worldmap_chart.add(year, A_countries)  #添加数据
    worldmap_chart.add('missing from word bank', B_countries)
    worldmap_chart.add('no data at this year', C_countries)
...
while float(year) < 1960 or float(year) > 2015:  #用while函数判断范围，若不在范围内，要求重新输入
    print('对不起，不能查询到该年的数据')
    print("----------------------")
    year=input("请输入需查询的具体年份:")
else:
    test_render_world_map(year)  #用自定义函数test_render_world_map绘图
